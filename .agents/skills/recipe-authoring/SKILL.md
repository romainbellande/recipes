...
2. **Draft the Recipe.** Inspect `src/content.config.ts`, `scripts/validate-recipes.mjs`, and a comparable Recipe. Use the established French Markdown structure, lowercase-kebab-case Recipe ID, controlled tags, weighed ingredients, and numbered preparation steps. Give every ingredient line exactly one quantity, the leading one: the incremental servings feature recalculates only the number at the start of the line (`scaleIngredient`), so any other figure on that line stays frozen and contradicts the line it annotates once Servings change. Move counts, package sizes, edible yields and ranges into the preparation steps, or drop them.
   - Bad: `- 160 g de tomme de Savoie (8 tranches épaisses)`, `- 500 g de haricots rouges égouttés (1 grande boîte)`, `- 4 à 8 feuilles de salade croquante` (doubling Servings reads `8 à 8 feuilles`).
   - Good: `- 160 g de tomme de Savoie`, `- 500 g de haricots rouges égouttés`, `- 8 feuilles de salade croquante`. A parenthesis carrying no number is safe: `- 2 c. à soupe d'huile d'olive (pour la sauce)`, `- 4 feuilles de salade croquante (sucrine)`. Product attributes are safe too: `- 90 ml de crème liquide entière (30 % de matière grasse)`.
   - Write fractions with the vulgar glyphs (`½ citron`, `1½ c. à soupe`), never `1/2`: the scaler reads `1/2` as the whole number 1 and doubles Servings into `2/2`.
   - Done when each ingredient is used by the method, every stated cook time has a corresponding step, and every ingredient line holds a single, leading quantity.
