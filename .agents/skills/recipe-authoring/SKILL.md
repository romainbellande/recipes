...
4. **Estimate nutrition.** Load and follow `recipe-nutrition-estimation`. Treat the Recipe's required `nutrition` front matter as a nutrition-estimation request: use USDA FoodData Central records, record assumptions and edible weights, reconcile the finished yield, and publish the Collection's per-100-g values.
